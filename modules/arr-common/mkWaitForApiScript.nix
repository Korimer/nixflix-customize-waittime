...
pkgs.writeShellScript "${serviceName}-wait-for-api" (
  ...
  ''
    ...
    for i in {1..${serviceConfig.waitForApiAttempts}}; do
      if ${
      ...
      echo "Waiting for ${capitalizedName} API... ($i/${serviceConfig.waitForApiAttempts})"
      sleep ${serviceConfig.sleepOnFailSeconds}
    done

    echo "${capitalizedName} API not available after ${serviceConfig.waitForApiAttempts * serviceConfig.sleepOnFailSeconds} seconds" >&2
    exit 1
  ''
)
